...
function Cell({ tool, index }: { tool: ToolMeta; index: number }) {
  ...
  // Previews are generated by `pnpm previews`, not committed, so a fresh
  ...
  return (
    ...
    >
      ...
      <div className="rule border bg-[var(--surface)] aspect-[4/3] overflow-hidden relative">
        ...
        {posterMissing && (
          <span className="absolute inset-0 grid place-items-center meta text-[var(--dim)]">
            {tool.name}
          </span>
        )}
        {tool.animated && onScreen && !posterMissing && (
          <video
            ref={videoRef}
