...
`pnpm dev` lists them on the index so a fresh checkout has something to look
at — that comes from `.env.development`, which never applies to a production
build. Delete the file, or set the flag to `0`, to develop against the real
(currently empty) index.

Preview images are generated by `pnpm previews` and are not committed, so
before you run it the grid shows each tool's name on a plain plate rather than
a poster. That is expected, not a broken build.
